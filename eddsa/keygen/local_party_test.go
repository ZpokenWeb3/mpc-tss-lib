...
func TestE2EConcurrentAndSaveFixturesBJJ(t *testing.T) {
	setUp("info")

	fmt.Printf("\n N: %d\n", tss.BabyJubJub().Params().N)
	fmt.Printf("\n P: %d\n", tss.BabyJubJub().Params().P)
	fmt.Printf("\n X: %d\n", tss.BabyJubJub().Params().Gx)
	fmt.Printf("\n Y: %d\n", tss.BabyJubJub().Params().Gy)
	fmt.Printf("\n A: %d\n", tss.BabyJubJub().Params().B)

	pk_ := iden3bjj.PublicKey{
		X: NewIntFromString("5299619240641551281634865583518297030282874472190772894086521144482721001553"),
		Y: NewIntFromString("16950150798460657717958625567821834550301663161624707787222815936182638968203"),
	}
	res := pk_.Point().InCurve()
	fmt.Printf("\n InCurve: %t\n", res)
	k := NewIntFromString("4914855077776770156877717678896562753238097178503088800574197080938319949305")
	a := pk_.Point().Mul(k, pk_.Point())
	fmt.Printf("\n a: %d %d\n", a.X, a.Y)
	res = a.InCurve()
	fmt.Printf("\n InCurve: %t\n", res)
