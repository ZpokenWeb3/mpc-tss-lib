...
func TestGenerateKeys_differentKeys(t *testing.T) {
	keyShares, _, err := NewKey(keyTestSize, keyTestK, keyTestL, &KeyMetaArgs{})
	if err != nil {
...
func TestGenerateKeys_validRandom(t *testing.T) {
	...
	keyShares, keyMeta, err := NewKey(keyTestSize, k, l, keyMetaArgs)
	if err != nil {
...
func TestGenerateKeys_validFixed(t *testing.T) {
	...
	keyShares, keyMeta, err := NewKey(keyTestFixedSize, k, l, keyMetaArgs)
	if err != nil {
		t.Errorf(fmt.Sprintf("%v", err))
	}
	docHash := sha256.Sum256([]byte(keyTestMessage))
