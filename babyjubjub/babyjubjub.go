...
var BabyJubJubParams = &CurveParams{
	...
	// N is the order of the curve: 21888242871839275222246405745257275088614511777268538073601725287587578984328
	// SubOrder is the order of the subgroup of the babyjub curve that contains the points that we use: 2736030358979909402780800718157159386076813972158567259200215660948447373041
	N:        iden3bjj.SubOrder,
	SubOrder: iden3bjj.SubOrder,
	A:        iden3bjj.A,
	D:        iden3bjj.D,
...
func (curve *BabyJubJubCurve) ScalarBaseMult(k []byte) (x, y *big.Int) {
	px, py := curve.ScalarMult(curve.Gx, curve.Gy, k)
	p := iden3bjj.Point{X: px, Y: py}
	// it must be on the curve, no need to check.
	if !curve.IsOnCurve(p.X, p.Y) {
		panic(fmt.Errorf("scalar mult to an ecpoint, coordinates %d %d", p.X, p.Y))
	}
	return p.X, p.Y
}

func (curve *BabyJubJubCurve) IsOnCurve(x *big.Int, y *big.Int) bool {
	p := &iden3bjj.Point{X: x, Y: y}
	ok := p.InCurve()
	return ok
}
